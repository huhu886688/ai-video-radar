...
不配置 API key 也能获取标题、发布时间、原视频链接和封面。配置 `YOUTUBE_API_KEY` 后，会额外获取播放量、点赞、评论和时长。
...
默认每天北京时间 08:30 运行一次，也可以在 GitHub Actions 页面手动点 `Run workflow`。
...
后续要增加频道，编辑 `scripts/fetch-latest.mjs` 里的 `sources` 数组即可。
...
- GitHub Actions 默认每天北京时间 `08:30` 更新。
- 综合热度公式：
...
当前仍是静态站架构，没有数据库。它适合低成本部署到 GitHub Pages。若后续要做用户订阅、长期指标快照、摘要队列和多平台 X/Podcast 聚合，再升级到后端数据库会更合适。
